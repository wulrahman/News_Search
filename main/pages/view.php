...
if ($view_total_count > 0) {
    ...
    $author_row = $get_content_article->get_author_name($row_content->site);
    
    $author = $author_row['name'];   

    $page_content[] = $row_content->id;

    $title_content = stripslashes(stripslashes($row_content->title));
    
    $title_content = $manipulation->limit_text(strip_tags($title_content),15);
    ...
    $tags_main = unserialize(json_decode($row_content->tags));
    ...
    $tags = array_values(array_filter($tags_new));
                        ...
                        <div class="similar_posts">
                            <div class="grid clearfix">
                                    ...
                                    foreach($similar_items as $key => $row_similar_content) {
                                        ...
                                        $title_similar  = $manipulation->limit_text(strip_tags(stripslashes(stripslashes($row_similar_content['title']))),8);

                                        $image_similar_main_url = $setting['main_url'].'/main/'.$row_similar_content['thumb_large_url'];
                                        
                                        $author_row = $get_content_article->get_author_name($row_similar_content['site']);
