			...
			<div class="row">
				<div class="col-lg-6 col-md-12">
					<div id="featured-slider" class="owl-carousel ts-overlay-style ts-featured">
                        ...
                        $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS `hash_id`, `id`, `author`, `tags`, `title`, `description`, `thumb_url`, `thumb_large_url`,  `width`, `source_url`, `published`, `timestamp` FROM `news` WHERE `id` NOT IN (".implode(",", $page_content).")  AND `publish` = '1' ORDER BY `timestamp` DESC LIMIT 0,3");
                        ...
                        if($total_count > 0) {
                            ...
                            while($row = mysqli_fetch_object($main_query)) {
                                ...
                                <div class="item" style="background-image:url(<?=$image_main_url?>)">
                                    <?php 
                                    $tags = array_values(array_filter($tags));
                                    if(count($tags) > 0 && !space($tags[0])) { ?>
                                        <a class="post-cat ts-orange-bg" href="#"><?=$tags[0]?></a> <?php
                                    }
                                    ?>
				...
				<div class="col-lg-3">
                    ...
                    $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS `hash_id`, `id`, `author`, `tags`, `title`, `description`, `thumb_url`,  `width`, `source_url`, `published`, `timestamp` FROM `news` WHERE `id` NOT IN (".implode(",", $page_content).")  AND `publish` = '1' ORDER BY `timestamp` DESC LIMIT 3,2");
                    ...
                    if($total_count > 0) {
                        ...
                        while($row = mysqli_fetch_object($main_query)) {
                            ...
                            $image_main_url = $setting['main_url'].'/main/'.$row->thumb_url;
                            $tags = unserialize(json_decode($row->tags)); 
                            $tags = array_values(array_filter($tags));?>
                            ...
                            <div class="ts-grid-box ts-grid-content"> 
                                ...
                                <div class="ts-post-thumb">
                                    <a href="<?=$setting['main_url']?>/?action=view&id=<?=$row->id?>">
                                        <img class="img-fluid" src="<?=$image_main_url?>" alt="">
                                    </a>
                                </div>
                                <div class="post-content">
                                    <h3 class="post-title">
					...
					<div class="post-list-item">
						...
						<div class="tab-content">
							<div role="tabpanel" class="tab-pane active ts-grid-box post-tab-list" id="home">
                                ...
                                $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS `hash_id`, `id`, `author`, `tags`, `title`, `description`, `thumb_url`,  `width`, `source_url`, `published`, `timestamp` FROM `news`  WHERE `id` NOT IN (".implode(",", $page_content).") AND `publish` = '1' ORDER BY `timestamp` DESC LIMIT 6,4");
                                ...
                                if($total_count > 0) {
                                    ...
                                    while($row = mysqli_fetch_object($main_query)) {
                                        ...
                                        <div class="post-content media">
                                            <img class="d-flex sidebar-img" src="<?=$image_main_url?>" alt="">
                                            <div class="media-body">
                                                <?php 
                                                if (count($tags) > 0) { ?>
                                                    <span class="post-tag">
                                                        <a href="#" class="green-color"> <?=$tags[0]?></a>
                                                    </span><?php
                                                }
                                                ?>
                                                <h4 class="post-title">
		...
		<div class="container">
			...
			<div class="ts-grid-box">
				...
				<div class="owl-carousel" id="hot-topics-slider">
                    ...
                    $array_color = array('ts-blue-light-heighlight', 'ts-green-heighlight', 'ts-pink-heighlight','ts-yellow-heighlight', 'ts-blue-heighlight', 'ts-green-heighlight');
                    
                    $color_count == 0;
                    
                    $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS SUM(`count`) AS `total_sum`,`category`  FROM `clicks` WHERE `timestamp` > DATE_SUB(CURDATE(), INTERVAL 200 HOUR) AND `type` = 'category' GROUP BY `category` ORDER BY `total_sum` DESC LIMIT 0, 10");

                    while($row = mysqli_fetch_object($main_query)) {
                        
                        $category_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS * FROM `categorys` WHERE `id` = '".$row->category."' LIMIT 0,1");
                        $category_row = mysqli_fetch_object($category_query);
                                                
                        $main_content = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS * FROM `news` WHERE `category`= '".$category_row->name."' AND `id` NOT IN (".implode(",", $page_content).")  AND `publish` = '1' ORDER BY `timestamp` DESC LIMIT 0,1");
                        ...
                        if($total_count > 0) {
                            ...
                            <div class="item <?=$array_color[$color_count]?> heighlight">
                                <div class="ts-post-thumb">
                                    <a href="<?=$setting['main_url']?>/?action=post&tag=<?=$category_row->name?>" >
                                        <img class="img-fluid" src="<?=$image_main_url?>" alt="">
                                    </a>
                                    <a class="post-cat" href="<?=$setting['main_url']?>/?action=post&tag=<?=$category_row->name?>"><?=$category_row->name?></a>
                                </div>

                                <div class="post-content">

                                    <h3 class="post-title">
                                        <a href="<?=$setting['main_url']?>/?action=post&tag=<?=$category_row->name?>"><?=$row_content->title?></a>
                                    </h3>
                                    <span class="post-date-info">
                                        <i class="fa fa-clock-o"></i>
                                        <?=$days?>
			...
			<div class="container">
				<div class="ts-grid-box ts-grid-box-heighlight">
					...
					<div class="owl-carousel" id="more-news-slider">
                        ...
                        $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS `id`, `hash_id`, `title`, `description`, `thumb_url`,  `width`, `source_url`, `published`, `timestamp`, `tags` FROM `news` WHERE `id` NOT IN (".implode(",", $page_content).") AND `publish` = '1' ORDER BY `timestamp` DESC LIMIT 10, 10");
                            ...
                            while ($row = mysqli_fetch_object($main_query)) { 
                                ...
                                <div class="ts-overlay-style">
                                    <div class="item">
                                        ...
                                        <?php
                                        if(count($tags) > 0 && !space($tags[0])) { ?>
                                            <a class="post-cat ts-green-bg" href="#"><?=$tags[0]?></a><?php
                                        }
                                        ?>
                                        <div class="overlay-post-content">
                                            <div class="post-content">
				...
				<div class="row">
					<div class="col-lg-9">
						...
						<div class="post-list">
                            ...
                            $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS SUM(`count`) AS `total_sum`,`article`  FROM `clicks` WHERE `timestamp` > DATE_SUB(CURDATE(), INTERVAL 200 HOUR) AND `type` = 'article' GROUP BY `article` ORDER BY `total_sum` DESC LIMIT 10, 10");
                            ...
                            while($row = mysqli_fetch_object($main_query)) {
                                ...
                                if($total_count > 0) {
                                    ...
                                    <div class="row mb-10">
                                        <div class="col-md-4">
                                            <div class="ts-post-thumb">
                                                <a href="<?=$setting['main_url']?>/?action=view&id=<?=$row_content->id?>">
                                                    <img class="img-fluid" src="<?=$image_main_url?>" alt="">
                                                </a>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-8">
                                            <div class="post-content">
                                                <h3 class="post-title md">
                                                    <a href="<?=$setting['main_url']?>/?action=view&id=<?=$row_content->id?>"><?=$row_content->title?></a>
                                                </h3>
                                                <ul class="post-meta-info">
                                                    <li>
                                                        <a href="#"><?=$row_content->author?></a>
                                                    </li>
                                                    <li>
                                                        <i class="fa fa-clock-o"></i>
                                                        <?=$days?>
                        ...
                        <div class="blog_section">
                            ...
                            <div class="section_content">
                                <div class="grid clearfix">
                                    ...
                                    $main_query = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS SUM(`count`) AS `total_sum`,`article`  FROM `clicks` WHERE `timestamp` > DATE_SUB(CURDATE(), INTERVAL 100 HOUR) AND `type` = 'article' GROUP BY `article` ORDER BY `total_sum` DESC LIMIT 0, 10");

                                    while($row = mysqli_fetch_object($main_query)) {

                                        $main_content = mysqli_query($setting["Lid"],"SELECT SQL_CALC_FOUND_ROWS  * FROM `news` WHERE `id` =  '".$row->article."' AND `id` NOT IN (".implode(",", $page_content).") AND `publish` = '1' ORDER BY `timestamp` DESC LIMIT 0,1");
                                        
                                        echo $total_count=array_pop(mysqli_fetch_row(mysqli_query($setting["Lid"],"SELECT FOUND_ROWS()")));
                                        ...
                                        if($total_count > 0) {
                                            ...
                                            <div class="card card_largest_with_image grid-item">
                                                ...
                                                <div class="card-body">
                                                    <div class="card-title"><a href="<?=$setting['main_url']?>/?action=view&id=<?=$row_content->id?>"><?=$row_content->title?></a></div>
                                                    <p class="card-text">?=$row_content->summary?></p>
