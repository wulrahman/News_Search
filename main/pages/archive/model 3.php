...
if($train_model_count == 0) {
    ...
    $article_query_clicked = mysqli_query($setting["Lid"], "(SELECT  `sentiment`, `author`, `readability`, (SELECT COUNT(`clicks`.`id`) FROM `clicks` WHERE `news`.`id` = `clicks`.`article` AND `identifier` IN ('".implode("','", $array_identifiers)."') AND `clicks`.`timestamp` >= NOW() - INTERVAL 30 DAY) AS `count` FROM `news` ORDER BY `count` DESC LIMIT 100)");


    $article_query_noclicked = mysqli_query($setting["Lid"], "(SELECT  `sentiment`, `author`, `readability`, (SELECT COUNT(`clicks`.`id`) FROM `clicks` WHERE `news`.`id` = `clicks`.`article`  AND `identifier` IN ('".implode("','", $array_identifiers)."') AND `clicks`.`timestamp` >= NOW() - INTERVAL 30 DAY) AS `count` FROM `news` ORDER BY `count` ASC LIMIT 100)");
    ...
    while($article_row_clicked = mysqli_fetch_array($article_query_clicked)) {
        ...
        $ai_data['input'][] = array(intval($article_row_clicked['sentiment']), intval($article_row_clicked['author']), intval($article_row_clicked['readability']));
    ...
    while($article_row_notclicked = mysqli_fetch_array($article_query_noclicked)) {
        ...
        $ai_data['input'][] = array(intval($article_row_notclicked['sentiment']), intval($article_row_notclicked['author']), intval($article_row_notclicked['readability']));

        $ai_data['output'][] = array($article_bool_click);

    }
    
    $brain = new Brainy($setting['learning_rate'], $setting['activation_fun']);

    // this is the input XOR matrix
    // remember to replace the zeros with -1 when you use TanH or Sigmoid
    $xor_in = array_splice($ai_data['input'], 0, 100);

    // this is the output of the XOR
    // remember to replace the zeros with -1 when you use TanH or Sigmoid
    $xor_out = array_splice($ai_data['output'], 0, 100);
    


            // forward the input and get the output
            $forward_response = $brain->forward($xor_in, $xor_out);
