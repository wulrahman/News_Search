...
class Syllables
{
    ...
    public static function syllableCount($strWord, $strEncoding = '')
    {
        ...
        if (Text1::letterCount(trim($strWord), $strEncoding) == 0) {
            return 0;
        }
        ...
        $strWord = Text1::lowerCase($strWord, $strEncoding);
    ...
    {
        $intSyllableCount = 0;
        $intWordCount = Text1::wordCount($strText, $strEncoding);
        $arrWords = explode(' ', $strText);
        for ($i = 0; $i < $intWordCount; $i++) {
            $intSyllableCount += self::syllableCount($arrWords[$i], $strEncoding);
        }
        ...
        for ($i = 0; $i < $intWordCount; $i++) {
            if (Syllables::syllableCount($arrWords[$i], $strEncoding) > 2) {
                if ($blnCountProperNouns) {
                    ...
                    $strFirstLetter = Text1::substring($arrWords[$i], 0, 1, $strEncoding);
                    if ($strFirstLetter !== Text1::upperCase($strFirstLetter, $strEncoding)) {
